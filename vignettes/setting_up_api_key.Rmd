...
An API key is a string that you communicates to the server (see [Wikipedia Page](https://en.wikipedia.org/wiki/Application_programming_interface_key)). The key is used by the server to identify you. This helps the server to manage your access, for example, giving access to specific services.
...
SHERPA/RoMEO has a limit of 500 queries per day without API key. Registering for an API key lets exceed this limit.
...
It is setup for the rest of your R session and `rromeo` will automatically use it when you call the different functions.
...
### Setting up your API key in an `.Renviron` file (**RECOMMENDED METHOD**)

`.Renviron` file follow the same loading rules as `.Rprofile` files, the only difference is that it is is a file that only purpose is to store environment variables. To use your API key in an `.Renviron` you have to type the following (note the absence of quotes):

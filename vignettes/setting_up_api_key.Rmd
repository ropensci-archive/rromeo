...
Like many other R packages that are API clients, `rromeo` lets you setup and use an API key.
...
It is setup for the rest of your R session and `rromeo` will automatically use it when you call the different functions. You can also use the `check_key()` function that should give the same result if your key is well set-up:
...
**NOTE:** This method is **NOT RECOMMENDED** but stil available for testing and development purposes. 
...
Furthermore using this method you have to specify it at each function call while the other methods shown above only needs to be setup once.

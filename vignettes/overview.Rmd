...
```{r}
res <- rr_journal_name("Biogeograph", qtype = "contains", multiple = TRUE)
```
...
res %>% 
  ...
  ggplot(aes(x = factor(file, c("preprint", "postprint", "pdf")), 
    ...
    xlab("") +
    scale_fill_manual(values = c("can" = "forestgreen", "restricted" = "orange", "cannot" = "firebrick", "unknown" = "gray"), na.value = "black")

res %>%
  ggplot(aes(x = "", fill = romeocolour)) +
    geom_histogram(stat = "count") + 
    coord_polar("y") +
    scale_fill_manual(values = c("gray" = "gray", "yellow" = "gold"), na.value = "black")
```
